<script lang="ts">
    ...
    const isLastIndex = (array: Array, index: number) => index == array.length - 1
    const getNeed = (marks: Attestation["attestation"], wish) => {
        const sum = marks.reduce((acc, {value}, index) => isLastIndex(marks, index) ? acc : acc + value, 0)
...
{#each attestation as {subject, attestation}, index (subject)}
    <section class="attestation">
        <h2 class="attestation__title">{subject}</h2>
        <ul class="attestation__summary">
            {#each attestation as {title, value}, i (title)}
                <li class="summary">
                    <p>
                    ...
                    <p class="summary__wish">
                        {#if needShow(attestation, i)}
                            (+{getNeed(attestation, wish)})
                        {/if}
                    </p>
                </li>
            {/each}
